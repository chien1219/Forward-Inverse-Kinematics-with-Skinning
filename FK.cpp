...
//      cout << "joint " << i << " " << orderStr << endl;
  ...
  computeLocalAndGlobalTransforms(jointRestTranslations, jointRestEulerAngles, jointOrientations, jointRotateOrders, 
      jointParents, jointUpdateOrder, jointInvRestGlobalTransforms/*not used*/, jointRestGlobalTransforms);
...
// Specifically, if xLocal is the homogeneous coordinate of a point expressed in the joint's local frame, and xParent is the homogeneous coordinate of the same point expressed in the frame of the joint's parent, we have:
    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // First, compute the localTransform for each joint, using eulerAngles and jointOrientationEulerAngles, and the "euler2Rotation" function.
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
	...
	int N = jointParents.size();

	double eulerRM[9], jointRM[9];
	vector<Mat3d> localTransMat3d(N);

	for (int i = 0; i < N; i++)
	{
		euler2Rotation(eulerAngles[i], eulerRM, rotateOrders[i]);
		euler2Rotation(jointOrientationEulerAngles[i], jointRM, XYZ);

		Mat3d eulerMat = Mat3d(eulerRM);
		Mat3d jointMat = Mat3d(jointRM);
		localTransMat3d[i] = jointMat * eulerMat;

		localTransforms[i] = AffineTransform4d(localTransMat3d[i], translations[i]);
	}

	for (int i = 0; i < N; i++) 
	{
		int j = jointUpdateOrder[i];

		// if root
		if (jointParents[j] == -1)
			globalTransforms[i] = localTransforms[j];
		else
			globalTransforms[i] = globalTransforms[jointParents[j]] * localTransforms[j];
	}
}
